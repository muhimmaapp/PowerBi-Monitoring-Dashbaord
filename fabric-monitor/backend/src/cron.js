...
async function runExtraction(tenants, daysBack = 1) {
  ...
  try {
    ...
    if (activities.length > 0) {
      ...
      for (const tenant of tenants) {
        ...
        logExtraction(tenant.id, toStr, tenantCount, "success");
      }
      console.log(`✅ Extraction complete: ${inserted} new events stored\n`);
    } else {
      console.log(`ℹ️  No new events found\n`);
      for (const tenant of tenants) {
        logExtraction(tenant.id, toStr, 0, "success");
      }
    }
  } catch (err) {
    console.error(`❌ Extraction failed: ${err.message}\n`);
    for (const tenant of tenants) {
      logExtraction(tenant.id, toDate.toISOString().split("T")[0], 0, "error", err.message);
    }
  }
}
